...
public class GetFpList extends HttpServlet {
		...
		String fpdm = req.getParameter("fpdm");
		String fphm = req.getParameter("fphm");
		...
		if (!ud.YzKey(req.getParameter("userKey"))) {
			...
			List<Fplist> nsrxxList = nsrxxDao.getFpList(fphm,fpdm);
			if (nsrxxList.size() == 0) {
				out.write("-1");
			} else {
				JSONArray array = new JSONArray();
				for (Fplist fp : nsrxxList) {
					JSONObject obj = new JSONObject();
					try {

						obj.put("zt", fp.getZt());
						obj.put("rq", fp.getRq());
						obj.put("fcjg_dm", fp.getFcjg_dm());
